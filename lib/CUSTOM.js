...
function saveCode() {
    try {
        ...
        showToast("Kod kaydedilemedi, depolama dolu olabilir.", "warning");
    }
}
...
function formatCode() {
    if (isViewSeparated) {
        ...
        try {
            ...
            showToast('Kod biçimlendirilemedi.', 'warning');
        }
    }
...
function revertCode() {
    showConfirmationModal('Tüm değişiklikleri ve kütüphane seçimlerini geri almak istediğinizden emin misiniz?', () => {
        selectedLibraries = [];
        ...
        if (isViewSeparated) {
            ...
            codeEditor.setValue(format(DEFAULT_CODE, "html"));
        }
        
        runCode(true);
        showToast('Değişiklikler ve kütüphaneler sıfırlandı', 'info');
    });
}
...
function saveFile() {
    // Dosyayı kaydederken kütüphaneler GEREKLİ
